...
export default async function SessionDetailPageEn({ params }: PageProps) {
  const { slug } = await params;
  const session = await getSessionBySlug(slug);
  if (!session) notFound();
  ...
  return (
    <article className="px-4 py-8 lg:px-8">
      <div className="mx-auto max-w-3xl">
        ...
        <Link href="/sesiones" className="mt-8 inline-block text-sm text-makina-pink hover:underline">
          ← Back to sessions
        </Link>
      </div>
    </article>
